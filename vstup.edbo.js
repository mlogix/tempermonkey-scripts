...
(function () {
  ...
  $(document).ready(function () {
    ...
    $(header).append(
      $('\
        <div class="offer-request" style="min-height: 150px">\
    ...
    $("#filter-action").on("click", function () {
      ...
      rows.each((_, row) => {
        const priority = $(row).find('.offer-request-other').text();
        const rate = Number.parseFloat($(row).find('.offer-request-kv > div').text().replace(',', '.'));
        if (priorities.includes(priority.toString().trim()) && rateMin <= rate && rate <= rateMax) {
          counter = counter + 1;
          $(row).show();
        } else {
          $(row).hide();
